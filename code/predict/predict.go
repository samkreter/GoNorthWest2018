...
func main() {
	...
	flag.StringVar(&modelDir, "modelDir", "", "The directory containing the model.")
	flag.StringVar(&varDir, "varDir", "", "The directory containing the input attributes.")
	flag.StringVar(&outDir, "outDir", "", "The output directory")

	flag.Parse()

	// Load the model file.
	f, err := ioutil.ReadFile(filepath.Join(modelDir, "model.json"))
	if err != nil {
		log.Fatal(err)
	}
	...
	if err := filepath.Walk(varDir, func(path string, info os.FileInfo, err error) error {
		...
		// Open any files.
		f, err := ioutil.ReadFile(filepath.Join(varDir, info.Name()))
		if err != nil {
			return err
		}
